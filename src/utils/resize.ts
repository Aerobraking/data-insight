...
export function resizeComplex(element: HTMLElement, listChildren: HTMLElement[]) {

    let elementSizeStart: ElementDimension = getCoordinatesFromElement(element);

    let listChildrenDimensions: ElementDimension[] = [];
    ...
    function initDrag(e: MouseEvent) {
        ...
        elementSizeStart = getCoordinatesFromElement(element);
        listChildrenDimensions = [];
        listChildren.forEach(c => {
            listChildrenDimensions.push(getCoordinatesFromElement(c));
        });
    ...
    function doDrag(e: MouseEvent) {
         ...
         * Calculate the scale factor
         */
        let elementSizeCurrent = getCoordinatesFromElement(element);
        let scaleW = elementSizeCurrent.w / elementSizeStart.w;
        let scaleH = elementSizeCurrent.h / elementSizeStart.h;

        // resize the children
        for (let i = 0; i < listChildren.length; i++) {
            const element: HTMLElement = listChildren[i];
            const dim = listChildrenDimensions[i];

            /**
             * Based on the distance to the origin of the resize rectangle, we calculate the new position
             */
            set3DPosition(element,
                dim.x + ((dim.x - elementSizeStart.x) * scaleW),
                dim.y + ((dim.y - elementSizeStart.y) * scaleH)
            );
            /**
             * the width/height act as vectors that can be scaled directly
             */
