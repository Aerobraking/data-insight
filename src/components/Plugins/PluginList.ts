...
export * from './Resize'
import  { RegisteredPlugins } from "./AbstractPlugin"

export function getPlugins() {
    RegisteredPlugins.forEach(p => {
        const panel = new p();
        console.log(p.name, panel.shortcut);
        // c.register(panel.shortcut.replaceAll("cmdorctrl", process.platform !== 'darwin' ? "ctrl" : "cmd"), () => {
        //     alert('Yes way!' + panel.shortcut);
        // });
    });
...
export const createContext = (options: { debounceTime: number, autoEnable: boolean }) => {
